...
async function deployStrictFreeSamplePrice() {
  ...
  console.log('SFTP Uploading updated src/app/input/page.js with strict 1,000 KRW free sample fallback...');
  
  await new Promise((res, rej) => {
    sftp.fastPut('src/app/input/page.js', '/tmp/saju_runtime_frontend/src/app/input/page.js', err => err ? rej(err) : res());
  });

  console.log('Verifying strict fallback logic in runtime input page...');

  const script = `
    grep -n "if (reportGrade === \\"free\\" || dbKey === \\"free_sample\\")" /tmp/saju_runtime_frontend/src/app/input/page.js
  ...
  console.log('STRICT FREE SAMPLE PRICE DEPLOY OUTPUT:\n', out);
...
deployStrictFreeSamplePrice().catch(err => console.error('ERROR:', err));
