...
async function deployPutRouteFixCorrect() {
  ...
  console.log('Creating directory /tmp/saju_runtime_frontend/src/app/api/admin/products...');
  await execCmd(conn, 'mkdir -p /tmp/saju_runtime_frontend/src/app/api/admin/products');

  const sftp = await new Promise((res, rej) => conn.sftp((err, s) => err ? rej(err) : res(s)));

  console.log('SFTP Uploading updated src/app/api/admin/products/route.js to runtime...');
  ...
  console.log('Rebuilding runtime app with fixed PUT handler...');
  ...
  console.log('PUT ROUTE FIX REBUILD OUTPUT:\n', out);
...
deployPutRouteFixCorrect().catch(err => console.error('ERROR:', err));
