...
console.log("=== Running Perfect Recovery for Pages 49 and 50 ===");

const lines = content.split('\n');

// 1. 첫 번째 case "ny_lucky_fashion": 의 인덱스 찾기
let fashionOffset = -1;
let currentLength = 0;
for (let i = 0; i < lines.length; i++) {
  const line = lines[i];
  if (line.trim() === 'case "ny_lucky_fashion":') {
    fashionOffset = currentLength;
    console.log(`Found first case "ny_lucky_fashion": at line ${i + 1}, offset ${fashionOffset}`);
    break;
  }
  currentLength += line.length + 1;
}

// 2. 첫 번째 case "ny_diet_presc": 의 인덱스 찾기
let dietOffset = -1;
currentLength = 0;
for (let i = 0; i < lines.length; i++) {
  const line = lines[i];
  if (line.trim() === 'case "ny_diet_presc":') {
    dietOffset = currentLength;
    console.log(`Found first case "ny_diet_presc": at line ${i + 1}, offset ${dietOffset}`);
    break;
  }
  currentLength += line.length + 1;
}

// 3. 두 번째 case "ny_intro_saju": 의 인덱스 찾기
let secondIntroOffset = -1;
let introCount = 0;
currentLength = 0;
for (let i = 0; i < lines.length; i++) {
  const line = lines[i];
  if (line.trim() === 'case "ny_intro_saju":') {
    introCount++;
    if (introCount === 2) {
      secondIntroOffset = currentLength;
      console.log(`Found second case "ny_intro_saju": at line ${i + 1}, offset ${secondIntroOffset}`);
      break;
    }
  }
  currentLength += line.length + 1;
...
if (fashionOffset === -1 || dietOffset === -1 || secondIntroOffset === -1) {
  console.error("Failed to detect one of the necessary case offsets!");
  process.exit(1);
}

// 4. replaceCase 헬퍼 함수
function replaceRange(startOffset, endCase, newContent) {
  const endIdx = content.indexOf(`case "${endCase}":`, startOffset + 10);
  if (endIdx === -1) throw new Error(`End case not found: ${endCase}`);
  
  content = content.slice(0, startOffset) + newContent + "\n\n      " + content.slice(endIdx);
}
...
const enrichedPage50 = `case "ny_diet_presc": {
        ...
        if (dayStemEl === "목" || dayStemEl === "木") {
          ...
          constitutionDesc = "소화기 계통의 힘을 쥐고 있으나, 세운의 열기가 강해져 위장이 쉽게 메마르고 체내 노폐물(습담)이 끈적하게 남기 쉬운 체질입니다. 비위가 약해지면 피로감 and 복부 팽만감을 자주 호소할 수 있습니다.";
...
// 5. 49페이지 교체 진행 (ny_lucky_fashion -> ny_diet_presc)
replaceRange(fashionOffset, "ny_diet_presc", enrichedPage49);

// 6. 변경된 내용 기반으로 50페이지 시작점 오프셋 재검출
const linesAfter49 = content.split('\n');
let newDietOffset = -1;
let currentLengthAfter49 = 0;
for (let i = 0; i < linesAfter49.length; i++) {
  const line = linesAfter49[i];
  if (line.trim() === 'case "ny_diet_presc":') {
    newDietOffset = currentLengthAfter49;
    console.log(`Re-detected case "ny_diet_presc": at line ${i + 1}, offset ${newDietOffset}`);
    break;
  }
  currentLengthAfter49 += line.length + 1;
}

if (newDietOffset === -1) {
  console.error("Failed to re-detect case ny_diet_presc offset!");
  process.exit(1);
}

// 7. 50페이지 교체 진행 (ny_diet_presc -> 두 번째 세트인 ny_intro_saju)
replaceRange(newDietOffset, "ny_intro_saju", enrichedPage50);

fs.writeFileSync(filePath, content, 'utf8');
console.log("=== Perfect Recovery Finished Successfully ===");
