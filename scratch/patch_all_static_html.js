...
async function patchAllStaticHtml() {
  ...
  console.log('Patching all static html and JS bundle files on remote server...');
  ...
  const script = `
    node -e "
          ...
          if (file.isDirectory()) {
            ...
            try {
              ...
              if (content.includes('무료 사주 보기') || content.includes('내 사주 무료 확인하기')) {
                console.log('Patching file: ' + fullPath);
      ...
      walkAndReplace('/home/www/saju-artpani/frontend/.next');
    "
...
patchAllStaticHtml().catch(err => console.error('ERROR:', err));
