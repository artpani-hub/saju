...
async function deployAndTestCleanPut() {
  ...
  console.log('SFTP Uploading cleaned src/app/api/admin/products/route.js...');
  ...
  console.log('Rebuilding runtime app with verified field schema...');
  ...
  const script = `
    ...
    NODE_PATH=/tmp/saju_runtime_frontend/node_modules node -e "
      ...
      const req = http.request({
        ...
        res.on('end', () => console.log('LIVE HTTP PUT TEST RESULT:', res.statusCode, body));
      });
  ...
  console.log('CLEAN DEPLOY AND TEST OUTPUT:\n', out);
...
deployAndTestCleanPut().catch(err => console.error('ERROR:', err));
