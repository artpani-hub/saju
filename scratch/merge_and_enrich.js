...
// 1. Add getPersonalizedSolution definition at the top level
const personalizedSolutionFunction = `
const getPersonalizedSolution = (name, text, category, dayStemEl) => {
  const cleanedText = text ? decodeURIComponent(text).replace(/<\/?[^>]+(>|$)/g, "") : "";
  ...
  if (category === "business" || category === "startup" || category === "trade") {
    ...
    analysis = \`의뢰인 \${name}님의 설비투자 및 사업장 확장 안건["\${cleanedText}"]에 대한 처방입니다. 장비 구입이나 대형 리모델링은 사주의 문서운(인성)과 장비 계약운(관성)의 흐름이 편안할 때 진행해야 고장이나 하자, 금융 비용의 폭증을 예방할 수 있습니다. \`;
    if (el === "목" || el === "木") {
    ...
    if (el === "목" || el === "木") {
      ...
      timing = "나를 지탱할 기틀과 뿌리가 채워지는 음력 9월(戌월) 가을철에 비로소 연봉을 높여 유리하게 이동할 수 있습니다.";
    ...
    if (el === "목" || el === "木") {
      ...
      timing = "나를 지탱할 든든한 토(土)와 금(金) 기운이 세력을 이루는 음력 8월~9월경 자금 수급이 매끄럽게 매듭지어집니다.";
...
// Insert getPersonalizedSolution after imports (before export const renderNewYearPageContent)
const importEndIndex = fileContent.indexOf('export const renderNewYearPageContent =');
fileContent = fileContent.slice(0, importEndIndex) + personalizedSolutionFunction + '\n' + fileContent.slice(importEndIndex);

console.log('Inserted getPersonalizedSolution definition');

// 2. Define the new dynamic ny_monthly case
      ...
      case "ny_monthly": {
        ...
        const getMonthBonus = (monthNum) => {
          ...
          if (dayStemEl === "목") {
            ...
            if (monthEl === "목") {
              bonusText = "수생목(水생목)의 활동 영역이 전개되어 추진해오던 신규 프로젝트나 창업 기획의 성과가 활짝 개화하는 운세입니다.";
              scoreModifiers.wealth = 1; scoreModifiers.love = 1; scoreModifiers.health = -1;
            } else if (monthEl === "화") {
        ...
        const baseMonthlyDetails = {
          ...
          3: {
            ...
            health: "위장 장애 and 소화 불량이 발생할 우려가 있으니 규칙적인 식사 시간을 유지하고, 식후 20분 동안 가볍게 대지를 딛으며 걷는 행동을 추천합니다.",
...
// Find where case "ny_monthly": is located in fileContent and replace it
const caseNyMonthlyMarker = 'case "ny_monthly":';
const startIdx = fileContent.indexOf(caseNyMonthlyMarker);

// Find the end of this case block by looking for the next case marker (ny_wealth_fortune)
const nextCaseMarker = 'case "ny_wealth_fortune":';
const endIdx = fileContent.indexOf(nextCaseMarker);

if (startIdx !== -1 && endIdx !== -1) {
  fileContent = fileContent.slice(0, startIdx) + dynamicNyMonthlyCase + '\n      ' + fileContent.slice(endIdx);
  console.log('Replaced ny_monthly with dynamicMonthly Fulfiller');
} else {
  console.log('Error: Could not locate ny_monthly or ny_wealth_fortune markers');
}

// 3. Prepare the 7 new/missing cases
...
const nyLuckyFashionDynamicCase = `
      case "ny_lucky_fashion": {
        const dayStemEl = sajuInfo?.day?.stemEl || "목";
        const deficientElName = prescriptions?.deficientEl || "목";
...
// Read other static files extracted
const nyCareerDetailedCode = fs.readFileSync(path.join(__dirname, 'ny_career_detailed.txt'), 'utf8');
const nySocialLifeCode = fs.readFileSync(path.join(__dirname, 'ny_social_life.txt'), 'utf8');
const nyRoadmap2030Code = fs.readFileSync(path.join(__dirname, 'ny_roadmap_2030.txt'), 'utf8');
const nyRoadmap2031Code = fs.readFileSync(path.join(__dirname, 'ny_roadmap_2031.txt'), 'utf8');
const nyDietPrescCode = fs.readFileSync(path.join(__dirname, 'ny_diet_presc.txt'), 'utf8');

// Combine all 7 cases
const allNewCases = [
  nyWealthPortfolioDynamicCase,
  nyCareerDetailedCode,
  nySocialLifeCode,
  nyRoadmap2030Code,
  nyRoadmap2031Code,
...
// Find default: in switch-case and insert these cases before it
const defaultCaseMarker = 'default:';
const defaultIdx = fileContent.indexOf(defaultCaseMarker);

if (defaultIdx !== -1) {
  fileContent = fileContent.slice(0, defaultIdx) + allNewCases + '\n\n      ' + fileContent.slice(defaultIdx);
  console.log('Inserted all 7 missing cases before default:');
} else {
  console.log('Error: Could not locate default: marker in switch statement');
}

// 4. Clean up any leftover file-level string artifacts (if any)
fs.writeFileSync(targetFilePath, fileContent, 'utf8');
console.log('Successfully saved modified renderNewYearPageContent.js');
