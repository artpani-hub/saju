...
conn.on('ready', () => {
  console.log('SSH Connection Ready for Deploy...');
  ...
  conn.sftp((err, sftp) => {
    ...
    filesToUpload.forEach(relPath => {
      ...
      const remotePath = `/home/www/artpani/backend_server/${relPath.replace(/\\/g, '/')}`;
      ...
      sftp.fastPut(localPath, remotePath, (uploadErr) => {
        ...
        if (completed === filesToUpload.length) {
          console.log('All files uploaded successfully. Building Next.js app on remote server...');
...
function triggerRemoteBuild(conn) {
  const buildCmd = `
    cd /home/www/artpani/backend_server &&
    npm run build &&
    pm2 reload artpani-backend
  `;
